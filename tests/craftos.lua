...
function M.creer(options)
  ...
  -- Deux formes de scenario coexistent dans le depot : l'une rend
  -- { entities = {...}, contraptions = {...} }, l'autre une simple liste de
  -- contacts. On accepte les deux plutot que d'imposer a l'une des familles de
  -- bancs de reecrire tous ses scenarios.
  local function normaliserScan(brut)
    if type(brut) ~= "table" then return {}, {} end
    if brut.entities ~= nil or brut.contraptions ~= nil then
      return brut.entities or {}, brut.contraptions or {}
    end
    return brut, {}
  end
  ...
  if options.radar then
    ...
    radar = {
      getContraptions = function()
        local _, contraptions = normaliserScan(options.radar(etat.horloge))
        return contraptions
      end,
      getRange = function() return options.radarPortee or 256 end,
    }
    radar[methode] = function()
      etat.scansRadar = (etat.scansRadar or 0) + 1
      -- Panne de radar declenchable en cours de scenario : c'est la seule
      -- facon de verifier qu'un systeme embarque survit a la perte de son
      -- capteur au lieu de s'arreter net.
      if etat.radarEnPanne then error("radar hors service", 0) end
      local entites = normaliserScan(options.radar(etat.horloge))
      return entites
    end
  end
  ...
  local extras = {}
  for nom, definition in pairs(options.peripheriques or {}) do extras[nom] = definition end

  --------------------------------------------- pilote (Create Aeronautics-like)
  -- Enregistre comme peripherique libre : aucune machinerie supplementaire, et
  -- le banc peut relire etat.navire et etat.consignes pour verifier ce qui a
  -- reellement ete commande.
  if options.pilote then
    ...
    local function noter(nom, valeur)
      etat.consignes[#etat.consignes + 1] =
        { commande = nom, valeur = valeur, t = etat.horloge }
    end
    extras[options.piloteNom or "aeronautics:helm_0"] = {
      type = options.piloteType or "aircraft_controller",
      methodes = {
  ...
  -- getMethods : un systeme qui resout ses methodes par introspection (l'arme
  -- embarquee, par exemple) ne trouverait rien sans lui.
  peripheralMock.getMethods = function(n)
    local enveloppe = peripheralMock.wrap(n)
    if type(enveloppe) ~= "table" then return {} end
    local noms = {}
  ...
  -- Le journal des impulsions permet de verifier qu'un leurre a bien ete
  -- largue ET que l'impulsion a bien ete refermee : l'etat final seul ne dit
  -- rien d'une impulsion breve.
  etat.redstone = {}
